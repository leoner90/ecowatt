...
class SendEmailModel 
{
    ...
    public function sendMail( $name, $surname, $msg, $email, $phone ) 
    {
       ...
       try {
           ...
           //Recipients
           $mail->setFrom('ecowatt.riga@gmail.com', 'ECOWATT');
           $mail->addAddress('leonid.gurockin@gmail.com', 'ECOWATT');
           $mail->addAddress('viktor.strogonow@yandex.ru', 'ECOWATT');
           $mail->addReplyTo($email, 'Client: '.$name.' '.$surname);
 
       
           //Content
           $mail->isHTML(true); 
           $mail->Subject = 'New Message From ECOWATT.lv';
           ...
           <body color=\"#999\"> 
                <hr>
               <h4> НОВОЕ СООБЩЕНИЕ ОТ ECOWATT.LV: </h4>
               <hr>
               <table>
